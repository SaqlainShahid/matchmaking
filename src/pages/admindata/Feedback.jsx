    ...
    <div className="space-y-6">
      <div className="card">
        <div className="card-content">
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Feedback Management</h2>
          <p className="text-gray-600 mt-1">Review, triage, and resolve feedback from order givers and providers.</p>
        </div>
      </div>
        ...
        <CardContent className="card-content">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div>
              ...
              <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Subject, message, or userId" />
            </div>
            <div>
                    ...
                    <tr key={f.id} className="border-t">
                      ...
                      <td className="p-2 align-top">
                        <div className="flex flex-wrap gap-2">
                          <Button size="sm" className="btn-secondary" onClick={() => updateStatus(f.id, 'reviewed')}>Review</Button>
                          <Button size="sm" className="btn-success" onClick={() => updateStatus(f.id, 'resolved')}>Resolve</Button>
                          <Button size="sm" className="btn-danger" onClick={() => updateStatus(f.id, 'dismissed')}>Dismiss</Button>
                        </div>
                        <div className="mt-2">
                          <label className="block text-xs text-gray-600 mb-1">Add admin note</label>
                          <div className="flex gap-2">
                            <Input value={noteById[f.id] || ''} onChange={(e) => setNoteById((p) => ({ ...p, [f.id]: e.target.value }))} placeholder="Internal note" />
                            <Button size="sm" onClick={() => addNote(f.id)}>Add</Button>
                          </div>
