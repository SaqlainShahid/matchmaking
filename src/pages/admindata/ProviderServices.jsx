    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm flex-1">
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">{t('Provider')} {t('Services')}</h2>
          <p className="text-gray-600 mt-2">{t('View services published by this provider.')}</p>
          <div className="mt-3 text-sm text-gray-700">
            <div><span className="text-gray-500">{t('Provider')}:</span> <span className="font-medium">{provider?.displayName || providerId}</span></div>
            <div><span className="text-gray-500">Email:</span> <span>{provider?.email || '—'}</span></div>
            <div><span className="text-gray-500">{t('Service Type')}:</span> <span>{provider?.serviceType || '—'}</span></div>
          </div>
        </div>
        <div className="flex gap-2 w-full md:w-auto">
          <Button variant="outline" onClick={() => navigate(-1)}>{t('Back')}</Button>
          <Link to={`/admin/providers`} className="btn btn-outline">{t('All')} {t('Providers')}</Link>
        </div>
      </div>

      <Card className="border border-gray-200 rounded-lg shadow-sm">
        <CardHeader className="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
          <CardTitle className="text-lg font-semibold text-gray-900">{t('Services')}</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            <div className="text-sm text-gray-600">{t('Loading')}</div>
          ) : services.length === 0 ? (
            <div className="text-sm text-gray-600">{t('No services found for this provider.')}</div>
          ) : (
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="text-left px-4 py-2">{t('Tags')}</th>
