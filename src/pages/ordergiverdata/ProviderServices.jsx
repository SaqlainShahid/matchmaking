...
const ProviderServices = () => {
  ...
  const providerName = useMemo(() => provider?.displayName || provider?.companyName || 'Provider', [provider]);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Button variant="outline" onClick={() => navigate('/providers')}>
            <FiArrowLeft className="h-4 w-4 mr-1" /> Back
          </Button>
          <h1 className="text-2xl font-semibold" style={{ color: 'var(--text-dark)' }}>{providerName}</h1>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="secondary" className="text-xs">Role: {(provider?.role || 'provider').replace('_',' ')}</Badge>
          {provider?.verified && <Badge className="text-xs">Verified</Badge>}
          <Button onClick={handleMessageProvider}>
            <FiMessageSquare className="h-4 w-4 mr-1" /> Message Provider
          </Button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-10 text-center text-gray-600">No published services</CardContent>
        </Card>
      ) : (
          ...
          {services.map((svc) => (
            <Card key={svc.id} className="hover:shadow-md transition-shadow">
              <CardHeader className="pb-2">
                <CardTitle className="text-lg truncate">{svc.title || 'Untitled Service'}</CardTitle>
              ...
              <CardContent className="space-y-3">
                <p className="text-sm text-gray-700 line-clamp-3">{svc.description || 'No description provided.'}</p>
                ...
                <div className="flex gap-2 pt-2">
                  <Button onClick={() => handleBuy(svc)}>
                    <FiShoppingCart className="h-4 w-4 mr-1" /> Buy Now
                  </Button>
      ...
      <Modal
        ...
        title={`Purchase ${selectedService?.title || 'Service'}`}
        footer={null}
      >
        {selectedService && (
      ...
      >
        <div className="space-y-2 text-sm">
          <p className="text-gray-700">Your payment has been processed successfully. A project will be created and the provider will be notified.</p>
          {selectedService && (
