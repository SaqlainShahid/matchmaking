  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Requests</h2>
        <p className="text-gray-600 mt-2">Browse open requests and submit professional quotes</p>
      </div>
      ...
      <Card className="border border-gray-200 rounded-lg shadow-sm">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            <input className="border rounded-md p-2" placeholder="Search" value={filters.q} onChange={(e) => setFilters({ ...filters, q: e.target.value })} />
            ...
            <input className="border rounded-md p-2" placeholder="Location" value={filters.location} onChange={(e) => setFilters({ ...filters, location: e.target.value })} />
      ...
      <Card className="border border-gray-200 rounded-lg shadow-sm">
        <CardHeader className="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg font-semibold text-gray-900">All Requests</CardTitle>
          </div>
        </CardHeader>
              ...
              filtered.map((req) => (
                <div key={req.id} className="p-4 flex items-center justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-gray-500">{req.serviceType} • {req.priority} • {String(req?.location?.address || '').slice(0, 40)}{String(req?.location?.address || '').length > 40 ? '…' : ''}</p>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="outline" onClick={() => { setSelectedRequest(req); setShowDetails(true); }}>View</Button>
                    <Button onClick={() => { setSelectedRequest(req); setShowQuoteModal(true); }}>Submit Quote</Button>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg w-full sm:max-w-lg md:max-w-2xl lg:max-w-3xl p-6 max-h-[85vh] overflow-y-auto">
            ...
            <div className="mt-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <div className="flex items-center">
                  <span className="w-28 text-gray-500">Service</span>
                  <span className="text-gray-900">{selectedRequest.serviceType || '—'}</span>
                </div>
                <div className="flex items-center">
                  <span className="w-28 text-gray-500">Urgency</span>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {selectedRequest.attachments.map((url, i) => (
                    ...
                    >
                      ...
                      <span className="text-blue-600 truncate group-hover:underline">Attachment {i + 1}</span>
                    </a>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <Button onClick={() => { setShowDetails(false); setShowQuoteModal(true); }}>Submit Quote</Button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-lg shadow-lg w-full sm:max-w-lg md:max-w-2xl lg:max-w-3xl p-6 max-h-[85vh] overflow-y-auto">
            ...
            <form onSubmit={submitInlineQuote} className="space-y-6 mt-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700 mb-1 block">Delivery Speed</label>
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700 mb-1 block">Estimated Duration</label>
                  <input className="border rounded-md p-2 w-full" placeholder="e.g., 2 days" value={quoteForm.duration} onChange={(e) => setQuoteForm({ ...quoteForm, duration: e.target.value })} />
                </div>
                <div>
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700 mb-1 block">Revisions</label>
              ...
              <div>
                ...
                <textarea className="border rounded-md p-2 w-full" rows="3" placeholder="Describe scope, terms, and additional details" value={quoteForm.note} onChange={(e) => setQuoteForm({ ...quoteForm, note: e.target.value })} />
              </div>
              <div>
      ...
      >
        <div className="space-y-2 text-sm">
          ...
          <p className="text-gray-600">You can track status under "Your Quotes".</p>
        </div>
      </Modal>
    </div>
  );
